...
export default class Mutator {
    ...
    static insert(Originals, data, _config) {
        console.log('insert Config',_config);
        ...
        try {
            ...
            //get updated doc in case hook changed data before insert
            var doc = this.find({_id: docId}).fetch()[0];
            dispatchInsert(config.optimistic, this._name, config._channels, doc);
    ...
    static update(Originals, selector, modifier, _config, callback) {
        console.log('update Config1',_config);
        const config = getMutationConfig(this._name, _config);
        console.log('update Config2',_config);
        if (!config.pushToRedis) {
