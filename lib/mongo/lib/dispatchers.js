...
const dispatchUpdate = function (optimistic, collectionName, channels, docIds, fields) {
    ...
    if (optimistic) {
        ...
        docIds.forEach(docId => {
            compensateForLatency(channels, collectionName, Events.UPDATE, uid, {_id: docId}, fields);
        });
    }
...
const dispatchRemove = function (optimistic, collectionName, channels, docIds) {
    ...
    if (optimistic) {
        ...
        docIds.forEach((docId) => {
            compensateForLatency(channels, collectionName, Events.REMOVE, uid, {_id: docId})
        });
    }
