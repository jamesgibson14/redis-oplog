...
export default class ObservableCollection {
    ...
    isEligibleByDB(doc) {
        if (this.testDocEligibility) {
            // Using Mingo to test query instead of using mongodb.
            var bool = new Mingo.Query(this.selector).test(doc);
            //console.log('isEligibleByDB',bool,this.selector, doc)
            return bool;
            // return !!this.collection.findOne(
            //     _.extend({}, this.selector, {_id: doc._id}),
            //     {fields: {_id: 1}}
            // )
        }
        return true;
    ...
    addById(doc) {
        if(_.isString(doc)){
            doc = this.collection.findOne({_id: doc}, this.options);
        }
